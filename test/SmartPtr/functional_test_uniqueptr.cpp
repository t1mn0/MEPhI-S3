...
TEST(TestUniquePtrGeneral, DefaultConstructorByString) {
  tmn::smart_ptr::UniquePtr<std::string> ptr;
  ASSERT_EQ(ptr.get(), nullptr);
}
...
TEST(TestUniquePtrGeneral, PointerConstructorByString) {
  ...
  tmn::smart_ptr::UniquePtr<std::string> ptr(raw_ptr);
  ASSERT_EQ(ptr.get(), raw_ptr);
}

TEST(TestUniquePtrGeneral, MoveConstructorWithInt) {
  tmn::smart_ptr::UniquePtr<int> ptr_1(new int(10));
  tmn::smart_ptr::UniquePtr<int> ptr_2(std::move(ptr_1));
  ASSERT_EQ(ptr_1.get(), nullptr);
  ASSERT_EQ((*ptr_2).value(), 10);
}

TEST(TestUniquePtrGeneral, MoveConstructorWithString) {
  tmn::smart_ptr::UniquePtr<std::string> ptr_1(new std::string("helloworld"));
  tmn::smart_ptr::UniquePtr<std::string> ptr_2(std::move(ptr_1));
  ASSERT_EQ(ptr_1.get(), nullptr);
...
TEST(TestUniquePtrGeneral, MoveAssignmentWithInt) {
  tmn::smart_ptr::UniquePtr<int> ptr_1(new int(10));
  tmn::smart_ptr::UniquePtr<int> ptr_2;
...
TEST(TestUniquePtrGeneral, MoveAssignmentWithString) {
  tmn::smart_ptr::UniquePtr<std::string> ptr_1(new std::string("cplusplus"));
  tmn::smart_ptr::UniquePtr<std::string> ptr_2;
  ptr_2 = std::move(ptr_1);
  ASSERT_EQ(ptr_1.get(), nullptr);
...
TEST(TestUniquePtrGeneral, Reset) {
  tmn::smart_ptr::UniquePtr<int> ptr(new int(42));
  ptr.reset(new int(10));
  ASSERT_EQ((*ptr).value(), 10);
}

TEST(TestUniquePtrGeneral, Swap) {
  tmn::smart_ptr::UniquePtr<int> ptr_1(new int(1));
  tmn::smart_ptr::UniquePtr<int> ptr_2(new int(2));
  ptr_1.swap(ptr_2);
...
TEST(TestUniquePtrGeneral, BoolConversion) {
  tmn::smart_ptr::UniquePtr<int> ptr_1(new int(42));
  tmn::smart_ptr::UniquePtr<int> ptr_2;
...
TEST(TestUniquePtrSpecial, MoveAssignment) {
  tmn::smart_ptr::UniquePtr<int[]> ptr_1(new int[25]);
  tmn::smart_ptr::UniquePtr<int[]> ptr_2;
  ptr_2 = std::move(ptr_1);
  ASSERT_EQ(ptr_1.get(), nullptr);
  ASSERT_NE(ptr_2.get(), nullptr);
...
TEST(TestUniquePtrSpecial, Swap) {
  ...
  tmn::smart_ptr::UniquePtr<int[]> ptr_1(raw_ptr_1);
  tmn::smart_ptr::UniquePtr<int[]> ptr_2(raw_ptr_2);
  ptr_1.swap(ptr_2);
...
TEST(TestUniquePtrSpecial, SwapNullptr) {
  int* raw_ptr = new int[10];
  tmn::smart_ptr::UniquePtr<int[]> ptr_1(raw_ptr);
  tmn::smart_ptr::UniquePtr<int[]> ptr_2;
  ptr_1.swap(ptr_2);
...
TEST(TestMakeUnique, MakeUniqueWithDefaultConstructor) {
  auto ptr = tmn::smart_ptr::make_unique<int>();
  ASSERT_NE(ptr.get(), nullptr);
}

TEST(TestMakeUnique, MakeUniqueWithArguments) {
  auto ptr = tmn::smart_ptr::make_unique<std::string>("Hello");
  ASSERT_NE(ptr.get(), nullptr);
  ASSERT_EQ(*(ptr.get()), "Hello");
}

TEST(TestMakeUnique, MakeUniqueWithSize) {
  auto ptr = tmn::smart_ptr::make_unique<int>(10);
  ASSERT_NE(ptr.get(), nullptr);
}

TEST(TestMakeUnique, MakeUniqueWithSizeZero) {
  try{
    auto ptr = tmn::smart_ptr::make_unique<int>(0);
  }
  catch(...){
    ASSERT_TRUE(true);
  }
